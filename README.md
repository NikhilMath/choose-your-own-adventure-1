...
- The Jordan arc (user's direction): the original one-scene payoff felt rushed, so it is now 4 scenes long with a real fork. `end_speech` (the meet-cute) leads to `jordan_drink`, then `jordan_weeks`, which forks: telling Jordan about the keyboard leads to `jordan_together` (the real ending, “The one who didn't laugh”); hiding it leads to `jordan_breakup`, which the user chose to make rejoin the main story at `plan` rather than stand alone, so a failed relationship becomes a second reason the protagonist goes back to obsessing over the keyboard.

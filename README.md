...
- Target structure (user's sketch, details pending): about 20 scenes total with about 10 different endings. The sketch shows one start scene on the left widening into a fan of branches toward the right. This replaces the earlier single-path, single-ending plan and the “single choices only” rule; confirm with the user how branches connect (and whether they rejoin a main line) before rebuilding.

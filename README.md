...
- `STORY`: only the approved opening (`arrival`), marked as a checkpoint with no choices. Its `prompt` tells visitors that the choose-your-own-adventure options are coming soon and Nikhil is working on them in the backend. It contains the plateau paragraph, a “A ticket pops up.” beat, and Daniel Hall’s (the CEO’s full name) 4:46 p.m. printer-driver ticket. No other scenes and no endings exist yet.

...
- The wizard branch (user's invention): asking Daniel about the keyboard leads to him giving it to you, it typing invoices by itself at 2 a.m., and the reveal that he is not Daniel Hall but Danal, seventh and most evil wizard of Xeon, a realm from which unwavering capitalism was first spawned. His endless good cheer is explained: he has not done real work in eleven years, the company is real, the money is real, the work never was. It pays off two earlier seeds, the line about his soul getting into the keyboard and “he has typed every invoice since.” The reveal forks three ways: take the magic (`end_wizard`, a real ending), refuse (`refuse`, which hands the player back to the heist at `outage`), or report him (`end_report`, a dead end). Note `end_founding` keeps its `end_` id although it is no longer an ending; renaming it would drop it from the tracking of anyone who already found it.
...
- The shared description says “two real endings and ten ways to miss them”; update it if that split changes.
...
- `STORY`: 37 scenes. Trunk: `arrival`, `office`, `plan`, `outage`, `grandmother`, `visit`, `audit`, `swap`, `choice`, `lie`. Mini-adventures: restart (`restart`, `ceremony`, `lemon`), walk away (`walk_away`, `resigned`, `draft`, `typo`), just ask (`ask`, `garage`, `declined`, plus the wizard branch `end_founding`, `invoices`, `wizard`, `refuse`), plus `printer_model`. Real endings (2): `end_plateau` (“The plateau”) and `end_wizard` (“Eleven years”). Dead ends (10): `end_spare`, `end_mom`, `end_method`, `end_red`, `end_running`, `end_sick`, `end_speech`, `end_sabbatical`, `end_founding` is NOT one (it continues), `end_borrowed`, `end_report`. Rejoin scenes: `printer_model` and `lemon` lead back to `office`; `draft` and `typo` to `plan`; `declined` and `refuse` to `outage`. Scenes have one to three buttons.
...
wizard  -> end_wizard (real ending), refuse, end_report
refuse  -> outage               (back into the heist)
```

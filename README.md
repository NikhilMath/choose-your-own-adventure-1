...
Build the spine (main storyline) first, then the diversions. A rough, unapproved outline is in the conversation history, not the code: office; plan; swap; complication; heist; guilt; keyboard at home; ending (still bored). Change it with the user before writing scenes. Ask the user to pick each next step, and follow the standing rule to summarize and suggest 2-3 next steps whenever starting a new path. If introducing inventory, conditional choices, or multiple stories, explicitly design state/save compatibility and revise this handoff.

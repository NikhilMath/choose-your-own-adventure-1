...
- The shared description says “three real endings and nine ways to miss them”; update it if that split changes.
...
- `STORY`: 37 scenes. Trunk: `arrival`, `office`, `plan`, `outage`, `grandmother`, `visit`, `audit`, `swap`, `choice`, `lie`. Mini-adventures: restart (`restart`, `ceremony`, `lemon`), walk away (`walk_away`, `resigned`, `draft`, `typo`), just ask (`ask`, `garage`, `declined`, plus the wizard branch `end_founding`, `invoices`, `wizard`, `refuse`), plus `printer_model`. Real endings (3): `end_plateau` (“The plateau”), `end_wizard` (“Eleven years”) and `refuse` (“The first no”). Dead ends (9): `end_spare`, `end_mom`, `end_method`, `end_red`, `end_running`, `end_sick`, `end_speech`, `end_sabbatical`, `end_borrowed`. `end_founding` and `end_report` continue despite their ids. Rejoin scenes: `printer_model` and `lemon` lead back to `office`; `draft` and `typo` to `plan`; `declined` and `end_report` to `outage`. Scenes have one to three buttons.
...
wizard  -> end_wizard (real ending), refuse (real ending), end_report
end_report -> outage            (back into the heist)
```

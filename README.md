...
- Story structure: the user wants every path to eventually converge on one main storyline; side branches return the protagonist to the main timeline rather than ending the story. This supersedes the fail-ending guidance below for the long term: `take_it_now` is currently a standalone ending but should be reworked to rejoin the main line once its convergence scene is defined with the user. Merged scenes must be route-neutral unless the engine gains state (see the merge rule in the editing contract).

...
- Link previews: `index.html` has Open Graph and Twitter card tags pointing at `og-image.png` (1200x630, made from an SVG; it says “16 ENDINGS,” so redo it if the ending count changes). LinkedIn caches previews; refresh with https://www.linkedin.com/post-inspector/.
...
- Keep the game easy to open and share as a small set of static files (`index.html`, `styles.css`, the icon files `favicon.svg`, `favicon.png`, `apple-touch-icon.png`, and the link-preview image `og-image.png`) with no build step, backend, or dependencies.
